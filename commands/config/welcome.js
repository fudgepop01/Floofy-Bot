...
module.exports = class Distance extends commando.Command {
	...
	async run(message) {
		...
		let settings = this.client.provider.get(message.guild, 'welcome', {});
		if (['enable', 'disable'].includes(args[1])) {
			settings.enabled = args[1] === 'enable';
			message.channel.send(`Welcome messages have been ${args[1] === 'enable' ? 'enabled' : 'disabled'}.`);
		}
		else if (['pm', 'public'].includes(args[1])) {
			if (!settings[args[2]]) settings[args[2]] = {};
			if (['enable', 'disable'].includes(args[2])) {
				settings[args[1]].enabled = args[2] === 'enable';
				message.channel.sendMessage(`${args[1].capitalize} welcome messages have been ${args[2] === 'enable' ? 'enabled' : 'disabled'}.`);
			}
			else if (args[1] === 'public' && args[2] === 'channel') {
// ;wecome public channel <channel>
				if (!args[3]) { return message.channel.send('Usage: `config welcome public channel id`'); }
				else {
					...
					message.channel.send(`The messages channel has been set to ${channel}.`);
				}
			}
			else if (args[2]) {
// ;welcome public/pm message
				...
				message.channel.send(`The ${args[1]} message has been set to:\n\`${msg.replace(/\[user]/g, 'USER')}\``);
			}
			else { message.channel.send(`Usage: \`welcome ${args[2]} enable/disable/message/channel\``); }
		}

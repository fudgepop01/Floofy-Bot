...
exports.run = async (bot, oldMember, newMember) => {
  // nicknames, roles
	...
	if (logs && logs.enabled && logs.channel) {
		...
		if (logs.fields ? logs.fields.nicknames !== false : !logs.fields && oldMember.nickname !== newMember.nickname && newMember.guild.channels.has(logs.channel)) {
		...
		if (logs.fields ? logs.fields.roles !== false : !logs.fields && oldMember.roles.size !== newMember.roles.size && newMember.guild.channels.has(logs.channel)) {
